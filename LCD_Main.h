...
// Graphics update inveral time(ms)
...
typedef enum LCD_Menu {
  ...
  ENUM_MENU_TEST_BUTTONS_SCREEN     = 1,
...
typedef struct LCD_Button {
  ...
  uint8_t               Visible; // Used with .press(false or true), keep as true or false
}LCD_Button;
...
typedef struct LCD_Screen_Data {
  LCD_Button    *ScreenButtonsArr;
  LCD_Text      *ScreenTextArr;
